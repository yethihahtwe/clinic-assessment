    ...
    protected static function getDomainTwoComponents(): array
    {
        ...
        foreach ($subdomains as $subdomain) {
            ...
            foreach ($questions as $question) {
                $questionLabel = $question->name;
                $questionSlug = $question->slug;
                $questionComponents[] = Radio::make('choices.'. $questionSlug)->label($i . '. ' . $questionLabel)->boolean()->inline()->inlineLabel(false)->live();
                $questionComponents[] = Group::make([
                    TextInput::make('choices.'. $questionSlug. '.m')->label('Male')->visible(fn (Get $get) => $get('choices.'. $questionSlug) == 1),
                ]);
                $i++;
            }
